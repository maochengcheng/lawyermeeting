...
public class AccessService extends BaseService{
    ...
    public Page<Access>  getAccesses(Access access, PageForm pageForm){
        Page<Access> accesses = accessRepository.findAll(SpecificationUtil.<Access>filter(access, pageForm).build(), pageForm.pageRequest());
...
//                APIError.e(400,"审核通知发送失败");
     ...
     * @return
     */
    public List<AppointmentSettings> obatinAppointmentSettings(Integer type){
        ...
        if(type.equals(6)){
            ...
            appointmentSettings = appointmentSettingsRepository.findAll(Specifications.<AppointmentSettings>and()
                    .eq("type", type).build(), Sort.by(Sort.Order.asc("period")));
        }
     ...
     * @return
     */
    public List<AppointmentSettings> wechatObatinAppointmentSettings(Integer type, Timestamp timestamp,String data){
...
//                hours="0"+String.valueOf(timestamp.getHours());
//            }else {
//                hours=String.valueOf(timestamp.getHours());
//            }
//            if(timestamp.getMinutes()<10){
//                minutes="0"+String.valueOf(timestamp.getMinutes());
//            }else {
//                minutes=String.valueOf(timestamp.getMinutes());
//            }
            ...
            for(int i=0;i<all.size();i++){
                ...
                Integer accessNumberCount = accessMapper.getAccessNumberCount(start, end);
