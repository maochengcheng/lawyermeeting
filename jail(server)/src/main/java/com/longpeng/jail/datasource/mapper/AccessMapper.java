...
public interface AccessMapper {
    ...
    @Select("select IFNULL(count(id),0) from access where appointment_time>=#{start} and appointment_time<=#{end} and presence_status=1")
    ...
    @Select("select appointment_time from access where attorney_id=#{attorneyId} order by appointment_time desc limit 0,1")
    Timestamp getAppointmentTime(Integer attorneyId);

    @Select("select IFNUll(count(id),0) from access  where appointment_time>=#{start} and appointment_time<=#{end} and certification!='REJECTED' and certification!='CANCEL' and presence_status=1 ")
    Integer getAccessNumberCount(String start,String end);
